use pg2any_lib::{
    destinations::{DestinationFactory, mysql::MySQLDestination, sqlserver::SqlServerDestination},
...
fn test_mysql_destination_update_with_old_data() {
    ...
    assert_eq!(old_data.get("id").unwrap(), &serde_json::Value::Number(serde_json::Number::from(1)));
    
    // Verify that new_data contains updated information
    assert!(new_data.contains_key("id"));
    assert!(new_data.contains_key("name"));
    assert_eq!(new_data.get("name").unwrap(), &serde_json::Value::String("new_name".to_string()));
}
...
fn test_delete_event_uses_old_data() {
    ...
    assert_eq!(old_data.get("name").unwrap(), &serde_json::Value::String("deleted_name".to_string()));
}
