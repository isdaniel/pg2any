...
impl ConfigBuilder {
    ...
    pub fn table_mapping<S: Into<String>>(mut self, table_name: S, mapping: TableMapping) -> Self {
        self.config.table_mappings.insert(table_name.into(), mapping);
        self
    }
    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.config.destination_connection_string.is_empty() {
            return Err(CdcError::config("Destination connection string is required"));
        }
        ...
        if self.config.streaming && self.config.protocol_version < 2 {
            return Err(CdcError::config("Streaming requires protocol version 2 or higher"));
        }
        
        if self.config.two_phase && self.config.protocol_version < 3 {
            return Err(CdcError::config("Two-phase commit requires protocol version 3 or higher"));
        }
...
impl TableMapping {
    ...
    where
        ...
        S2: Into<String>,
    {
        self.column_mappings.insert(source.into(), destination.into());
        self
    }
