...
fn load_config_from_env() -> Result<Config, Box<dyn std::error::Error>> {
    ...
    let source_password = std::env::var("CDC_SOURCE_PASSWORD").unwrap_or_else(|_| "postgres".to_string());
    ...
    tracing::info!("Source PostgreSQL connection string: {}", source_connection_string);
    ...
    let dest_password = std::env::var("CDC_DEST_PASSWORD").unwrap_or_else(|_| "cdc_password".to_string());
    
    let destination_connection_string = match dest_type {
        DestinationType::MySQL => {
            format!("mysql://{}:{}@{}:{}/{}", dest_user, dest_password, dest_host, dest_port, dest_db)
        }
        DestinationType::SqlServer => {
            format!("mssql://{}:{}@{}:{}/{}", dest_user, dest_password, dest_host, dest_port, dest_db)
        }
        _ => {
    ...
    tracing::info!("Destination: {:?} at {}@{}:{}/{}", dest_type, dest_user, dest_host, dest_port, dest_db);
    
    // CDC-specific configuration
    let replication_slot = std::env::var("CDC_REPLICATION_SLOT").unwrap_or_else(|_| "cdc_slot".to_string());
    ...
    tracing::info!("CDC Config - Slot: {}, Publication: {}, Protocol: {}", 
        replication_slot, publication, protocol_version);
