...
impl CdcClient {
    ...
    async fn process_pending_transaction_files(
        ...
        // The recovery loop below decrements the counter for every file it processes: normal finalize and dedup-skipped duplicates alike both decrement, so the counter stays accurate as it drains toward 0.
