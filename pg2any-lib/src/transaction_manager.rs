...
impl TransactionManager {
    ...
    /// Atomically saturating-decrement the pending counter, returning the new value.
    /// saturating_sub guards against underflow if a recovery path uncounts a tx that
    /// wasn't counted at startup (the atomic itself never wraps to usize::MAX).
    fn decrement_pending_count(&self) -> usize {
    ...
    pub async fn commit_transaction(
        ...
        // Whether this tx was already counted in pending_tx_count (its pending file
        // already exists from a prior commit). Used below to avoid double-incrementing
        // on a duplicate/recovery commit that overwrites the existing pending file.
        let already_committed = fs::metadata(&pending_metadata_path).await.is_ok();
        ...
        // Only count a newly-created pending file. A duplicate/recovery commit that
        // overwrote an existing pending file (with no execution progress) was already
        // counted, so re-incrementing would inflate the monitoring counter.
        if !already_committed {
...
impl TransactionManager {
    ...
    pub async fn process_transaction_file(
        ...
        if let Some(commit_lsn) = pending_tx.metadata.commit_lsn {
            ...
            if commit_lsn.0 <= current_flush_lsn && current_flush_lsn > 0 {
                ...
                // Keep the monitoring counter accurate in real-time: this counted
                // pending tx is being removed without going through finalize.
    ...
    async fn finalize_transaction_file(
        ...
        // Decrement to match the increment in `commit_transaction`: any transaction
        // that reached the committed/pending state was counted, so it must be
        // uncounted here regardless of `commit_lsn`.
        let pending_count = self.decrement_pending_count();
