...
impl ConnectionPool {
    ...
    pub fn new(connection_string: String, pool_size: usize, timeout: Duration) -> Self {
        Self {
    ...
    pub async fn init(&mut self) -> Result<()> {
        for i in 0..self.pool_size {
            let mut conn = PostgresConnection::new(self.connection_string.clone(), self.timeout);
            conn.connect().await?;
            self.connections.push(conn);
            debug!("Initialized connection {} of {}", i + 1, self.pool_size);
        }
        info!("Initialized connection pool with {} connections", self.pool_size);
        Ok(())
    }

    /// Get an available connection from the pool
    pub fn get_connection(&mut self) -> Result<&mut PostgresConnection> {
        self.connections.first_mut().ok_or_else(|| {
            CdcError::generic("No connections available in pool")
        })
    }
...
impl Drop for ConnectionPool {
    fn drop(&mut self) {
        // Note: This is synchronous drop, so we can't call async methods
        // In practice, you might want to use a more sophisticated shutdown mechanism
