...
async fn test_relay_log_end_to_end() {
    use pg2any_lib::relay_log::{RelayLogManager, RelayLogConfig};
    ...
    for i in 1..=3 {
        ...
        data.insert("name".to_string(), serde_json::Value::String(format!("test_{}", i)));
    ...
    // Write events
    for event in &events {
        writer.write_event(event.clone(), event.lsn.clone()).await.unwrap();
    }
    ...
    for (original, read) in events.iter().zip(read_events.iter()) {
        ...
        if let (EventType::Insert { schema: orig_schema, table: orig_table, .. },
                EventType::Insert { schema: read_schema, table: read_table, .. }) = 
