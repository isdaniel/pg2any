#!/bin/bash
#
# PGBench Chaos Test Runner - SQLite Destination
...
SQLITE_DB_PATH="/app/data/cdc_target.db"
...
# Connection string
POSTGRES_CONNSTRING="postgresql://${POSTGRES_USER}@${POSTGRES_HOST}:${POSTGRES_PORT}/${POSTGRES_DB}"
...
# Results file
RESULTS_FILE="$SCRIPT_DIR/pgbench_chaos_sqlite_results_$(date +%Y%m%d_%H%M%S).log"
...
test_sqlite_connection() {
    ...
    if docker exec "$CDC_CONTAINER" sqlite3 "$SQLITE_DB_PATH" "SELECT 1;" > /dev/null 2>&1; then
        log_success "SQLite connection successful."
        return 0
    else
...
verify_replication() {
    ...
    log_info "Current SQLite row count: $current_count / Expected: $EXPECTED_ROW_COUNT"
...
main() {
    ...
    log_section "PGBench Chaos Integration Test (SQLite)"
    log_info "Timestamp: $(date '+%Y-%m-%d %H:%M:%S')"
    log_info "Container under test: $CONTAINER_NAME"
    log_info "SQLite DB path: $SQLITE_DB_PATH"
    echo "" | tee -a "$RESULTS_FILE"
    ...
    if [ $benchmark_result -eq 0 ] && [ "$verification_passed" = true ]; then
        log_success "PGBench chaos test completed successfully! (SQLite)"
        log_success "All $EXPECTED_ROW_COUNT rows replicated to SQLite"
