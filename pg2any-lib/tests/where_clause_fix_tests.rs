...
fn test_update_where_clause_uses_old_data() {
    ...
    old_data.insert("id".to_string(), serde_json::Value::Number(serde_json::Number::from(42)));
    old_data.insert("email".to_string(), serde_json::Value::String("old.email@example.com".to_string()));

    let mut new_data = HashMap::new();
    new_data.insert("id".to_string(), serde_json::Value::Number(serde_json::Number::from(42)));
    new_data.insert("email".to_string(), serde_json::Value::String("new.email@example.com".to_string()));
    new_data.insert("name".to_string(), serde_json::Value::String("Updated Name".to_string()));
    new_data.insert("age".to_string(), serde_json::Value::Number(serde_json::Number::from(30)));
    ...
    assert_eq!(old_data.get("id").unwrap(), &serde_json::Value::Number(serde_json::Number::from(42)));
    assert_eq!(old_data.get("email").unwrap(), &serde_json::Value::String("old.email@example.com".to_string()));
    ...
    assert_eq!(new_data.get("email").unwrap(), &serde_json::Value::String("new.email@example.com".to_string()));
...
fn test_delete_where_clause_uses_old_data() {
    let mut old_data = HashMap::new();
    old_data.insert("id".to_string(), serde_json::Value::Number(serde_json::Number::from(99)));
    old_data.insert("username".to_string(), serde_json::Value::String("user_to_delete".to_string()));
...
fn test_update_fallback_when_no_old_data() {
    let mut new_data = HashMap::new();
    new_data.insert("id".to_string(), serde_json::Value::Number(serde_json::Number::from(123)));
    new_data.insert("name".to_string(), serde_json::Value::String("New Name".to_string()));
